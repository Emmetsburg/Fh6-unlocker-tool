...
static bool ImGui_ImplDX11_CompileShader(const char* source, const char* target, ID3DBlob** out_blob)
{
    ID3DBlob* error_blob = nullptr;
    const HRESULT hr = D3DCompile(source, strlen(source), nullptr, nullptr, nullptr, "main", target, 0, 0, out_blob, &error_blob);
    if (error_blob)
        error_blob->Release();
    return SUCCEEDED(hr) && out_blob && *out_blob;
}

static bool ImGui_ImplDX11_HasDeviceObjects(ImGui_ImplDX11_Data* bd)
{
    return bd != nullptr &&
...
static const char* ImGui_ImplDX11_VertexShaderSource =
...
static const char* ImGui_ImplDX11_PixelShaderSource =
    "struct PS_INPUT { float4 pos : SV_POSITION; float4 col : COLOR0; float2 uv : TEXCOORD0; };"
    "sampler sampler0;"
    "Texture2D texture0;"
...
static bool ImGui_ImplDX11_EnsureDeviceObjects(ImGui_ImplDX11_Data* bd)
{
    if (ImGui_ImplDX11_HasDeviceObjects(bd))
        return true;
    if (!ImGui_ImplDX11_CreateDeviceObjects())
        return false;
    return ImGui_ImplDX11_HasDeviceObjects(bd);
}
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (!ImGui_ImplDX11_EnsureDeviceObjects(bd))
        return;
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        if (!ImGui_ImplDX11_CompileShader(ImGui_ImplDX11_VertexShaderSource, "vs_4_0", &vertex_shader_blob))
            return false;
        if (bd->pd3dDevice->CreateVertexShader(vertex_shader_blob->GetBufferPointer(), vertex_shader_blob->GetBufferSize(), nullptr, &bd->pVertexShader) != S_OK)
        {
        ...
        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, vertex_shader_blob->GetBufferPointer(), vertex_shader_blob->GetBufferSize(), &bd->pInputLayout) != S_OK)
        {
            vertex_shader_blob->Release();
            return false;
        }
    ...
    // Create the pixel shader
    {
        ...
        if (!ImGui_ImplDX11_CompileShader(ImGui_ImplDX11_PixelShaderSource, "ps_4_0", &pixel_shader_blob))
            return false;
        if (bd->pd3dDevice->CreatePixelShader(pixel_shader_blob->GetBufferPointer(), pixel_shader_blob->GetBufferSize(), nullptr, &bd->pPixelShader) != S_OK)
        {
...
void ImGui_ImplDX11_NewFrame()
{
    ...
    ImGui_ImplDX11_EnsureDeviceObjects(bd);
}
