...
float4 PS_Glass(VSOut i) : SV_Target
{
    ...
    float3 glass = tint_color.rgb;
    glass = (glass - 0.5) * contrast + 0.5 + brightness * 0.12;
    glass = max(glass, 0.0);
    ...
    float panel_alpha = saturate(tint_opacity * 0.55 + border_intensity * 0.08 + highlight * 0.04);
    float a   = shape * fade * panel_alpha;
